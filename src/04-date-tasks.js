...
function timeSpanToString(startDate, endDate) {
  const diferent = Math.abs(endDate.getTime() - startDate.getTime());
  const resDate = new Date(diferent);
  let hours = resDate.getUTCHours();
  let minutes = resDate.getUTCMinutes();
  let seconds = resDate.getUTCSeconds();
  let mill = resDate.getUTCMilliseconds();

  if (hours <= 9) hours = `0${hours}`;
  if (String(minutes).length < 2) minutes = `0${minutes}`;
  if (String(seconds).length < 2) seconds = `0${seconds}`;
  if (String(mill).length < 3) {
    mill = `00${mill}`;
